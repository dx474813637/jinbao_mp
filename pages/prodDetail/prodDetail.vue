<template>
	<view class="w">
		...
		<u-popup v-model="show" mode="center" width="90%" closeable close-icon-color="#fff" border-radius="20">
			...
			<u-swiper :list="list" height="1000" img-mode="aspectFit" mode="number" :autoplay="false"
				:current="curIndex" bg-color="#111"></u-swiper>
		...
		<view class="tabbar-w safe-area-inset-bottom" v-if="sh == 0">
			<view class="tabbar">
				...
				<navigator url="/pages/cart/cart" class="tabbar-item tab-icon-btn">
					<div class="item-icon"><u-badge type="error" :count="count" :offset="[-5, -10]"
							size="mini"></u-badge><u-icon name="cart" custom-prefix="custom-icon"></u-icon></div>
					<div class="item-icon">购物车</div>
				</navigator>
				<view class="tabbar-item tab-btn buy-btn">
					<u-button type="error" shape="circle" @click="inCart"><u-icon name="shopping-cart-fill"
							color="#fff"></u-icon><text class="btn-text">加入购物车</text></u-button>
				</view>
			</view>

		</view>
		<view class="swiper">
			<u-swiper :list="list" height="500" img-mode="aspectFit" mode="number" border-radius="0" interval="5000"
				indicator-pos="bottomRight" @click="handleChangeShow" bg-color="#fff"></u-swiper>
		</view>
		<view class="bg-white u-p-30 u-m-b-30">
			<view class="info-row info-price">
				...
				<view class="discount-right">
					<!-- <navigator url="/pages/discount/shopCoupon" class="discount-btn">旺铺优惠劵<u-icon
							name="arrow-right"></u-icon></navigator> -->
				</view>
			</view>
		...
		<view class="prod-info">
			<view class="info-card">
				<view class="info-row single-info" v-if="info.brand">
					<view class="info-label">品牌</view>
					<view class="info-value">{{info.brand}}</view>
				</view>
				<view class="info-row single-info" v-if="info.model">
					<view class="info-label">规格型号</view>
					<view class="info-value">{{info.model}}</view>
				</view>
				<view class="info-row single-info">
					<view class="info-label">库存</view>
					<view class="info-value">{{info.stock}}{{info.unit}}</view>
				</view>
				<view class="info-row single-info" v-if="info.producer">
					<view class="info-label">生产商</view>
					<view class="info-value">{{info.producer}}</view>
				</view>
				<view class="info-row single-info" v-if="info.send_regional">
		...
		<!-- <view class="prod-seller u-p-30 bg-white u-m-b-20">
			<view class="u-flex u-flex-items-center u-flex-between">
				<view class="item u-flex u-flex-items-center">
					<view class="item u-m-r-10">
						<u-image :src="logo" width="50px" height="50px" shape="circle"></u-image>
					</view>
					<view class="item">
						<view class="text-black u-font-32 u-m-b-8 u-line-1">{{sellerInfo.company}}</view>
						<view class="labeltag u-radius-3">
							认证商家
						</view>
					</view>
				</view>
				<view class="">
					<u-button type="error" plain shape="circle" size="mini" @click="$u.route('/pages/index/index')">进店逛逛</u-button>
				</view>
			</view>
		</view> -->
		<view class="bg-white u-p-30"> 
			<u-parse :html="info.intro" v-if="info.intro" :tagStyle="{
				img: 'width: 100%',
				p: 'line-height: 25px'
			}"></u-parse>
		</view>
		...
		<movable-area class="video-w safe-area-inset-bottom" v-if="live_src" :style="{
					'width': vWidth + 'rpx',
					'height': movableH + 'px',
				}" scale-area>
			<movable-view class="video" :style="{
						'width': vWidth + 'rpx',
						'height': vHeight + 'rpx',
						'pointer-events': 'auto'
					}" @click="gotoLive" direction="vertical" inertia :y="movableH*2 - vHeight + 'rpx'" out-of-bounds friction="4">
				<video id="myVideo" :src="live_src" @error="videoErrorCallback" :controls="false" autoplay poster=""
					style="width: 100%;height: 100%;" object-fit="fill" @loadedmetadata="handleGetmeadata"
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				let res = await this.$http.get('product_detail', {
					params: {
						id: this.id
					}
				})
			...
			homeUrl() {
				this.$http
					...
					.then(res => {
						if (res.data.code == 1) {
							this.homeInfo = res.data.list
							uni.setStorageSync('home', {
								title: res.data.list.title,
								imageurl: res.data.list.imageurl
							})
							uni.setNavigationBarColor({
								frontColor: this.homeInfo.frontcolor,
								backgroundColor: this.homeInfo.bgcolor,
								animation: {
									duration: 400,
									timingFunc: 'easeIn'
								}
