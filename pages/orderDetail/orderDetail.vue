	...
	<view class="w">
		...
		<view class="order-detail" style="padding-bottom: 30rpx">
			...
			<view class="info-row">
				...
				<view class="info-value">
				{{list.toTools}}
					<text class="text-primary u-m-l-20" v-if="list.order_rz_pay" @click="$u.route({url: '/pages/order_bank/order_bank', params: {id: list.order_rz_pay.id}})">查看融资详情</text>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			async RzConfirm() {
				...
				let res = await this.$http.get('rz_check_tran', {
					params: {
						order_id: this.id
					}
				})
			...
			async buyBtn() {
				...
				const res = await this.$http.get('rz_pay', {params: {
