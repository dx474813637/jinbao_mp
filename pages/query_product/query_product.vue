	...
	<view class="u-p-40"> 
		...
		<view class="card-w u-radius-8 u-p-20">
			...
			<view class="bg-white u-p-20 u-radius-8">
				<view class="u-p-10 u-flex u-flex-column u-flex-items-center u-m-b-40">
				...
				<view class="card-info u-flex u-flex-items-start u-flex-wrap u-p-30 u-p-l-40 u-p-r-40 u-radius-8 u-m-b-10">
					...
					<view class="item u-flex-1 u-flex u-flex-column u-flex-items-center">
						...
						<view class="value u-font-40">{{list.credit_term || '-'}}{{list.credit_mode | date2Str}}</view>
					</view> 
					<view class="item u-flex-1 u-flex u-flex-column u-flex-items-center">
					...
					<view class="item u-flex-1 u-flex u-flex-column u-flex-items-center">
						...
						<view class="value u-font-40">{{list.loan_term || '-'}}{{list.credit_mode | date2Str}}</view>
					</view>
				</view>
				...
				<view class="u-p-40 u-m-t-20 u-p-l-60 u-p-r-60">
					...
					<view class="">
						<u-button type="primary"  plain @click="$u.route('/pages/pacc_query/pacc_query_list')">查看历史授信</u-button>
					</view>
				</view>
				 <view>
					 <u-image
...
<script>
	...
	export default {
		...
		methods: {
			async getData() {
				const res = await this.$http.get('query_product_detail', {params: {product_id: this.pid}})
				if(res.data.code == 1) {
			...
			paccClick() {
				uni.navigateTo({
					url: `/pages/pacc_query/pacc_query_edit?id=${this.pid}`
				})
			},
		}
	}
