...
import {getAccount, getCpy} from '@/utils/userCard'
Vue.use(Vuex)
...
const store = new Vuex.Store({
    ...
    mutations: {
		...
		setAccInfo(state, info={}) {
			state.account = info
		}, 
		setCpyInfo(state, info={}) {
	...
	actions:{
		...
		async initBankData({commit, state}) {
			state.bankLoading = true
			const res = await http.get('transfer_random_amount_detail')
			state.bankLoading = false
			if(res.data.code == 1) {
				commit('setBankData', res.data.list.bank_list ) 
			}
		},
		async initAreaData({commit, state}) {
			state.areaLoading = true
			const res = await http.get('get_regional')
			state.areaLoading = false
			if(res.data.code == 1) {
				commit('setAreaData', res.data.list.items.map(ele => {
					return {
						...ele,
						children: res.data.list[`items${ele.code}`]
					}
				}) )  
			}
		},
